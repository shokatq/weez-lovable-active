        ...
        {messages.map((message) => (
          <div
            ...
            className={`flex gap-4 items-start ${
              message.isUser ? "" : "" // No justify needed, alignment comes from avatar order
            }`}
          >
            {!message.isUser && (
                ...
                {message.files && message.files.length > 0 && (
                  <div className="mt-4 space-y-2 border-t border-gray-700 pt-3">
                    {message.files.map((file) => (
                      <div key={file.id} className="bg-gray-700/50 rounded-lg p-3 flex items-center gap-3 border border-gray-600/50">
