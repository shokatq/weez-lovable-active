...
-- Create or update space_members table  
...
-- Create or update files table for space file management
...
DROP POLICY IF EXISTS "Team members can view spaces" ON public.spaces;
CREATE POLICY "Team members can view spaces" ON public.spaces
  FOR SELECT USING (
    EXISTS (
...
DROP POLICY IF EXISTS "Team admins can manage spaces" ON public.spaces;
CREATE POLICY "Team admins can manage spaces" ON public.spaces
...
CREATE POLICY "Space members can view space membership" ON public.space_members
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Team admins can manage space membership" ON public.space_members
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.spaces s
...
-- RLS Policies for files (role-based permissions)
