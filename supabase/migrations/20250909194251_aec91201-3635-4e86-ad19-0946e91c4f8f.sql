...
DO $$ 
BEGIN
  -- Add status column to space_members if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'space_members' AND column_name = 'status') THEN
    ALTER TABLE public.space_members ADD COLUMN status text DEFAULT 'active';
  END IF;
  
  -- Add email column to space_members if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'space_members' AND column_name = 'email') THEN
    ALTER TABLE public.space_members ADD COLUMN email text;
  END IF;
  
  -- Add role column to space_members if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'space_members' AND column_name = 'role') THEN
    ALTER TABLE public.space_members ADD COLUMN role app_role DEFAULT 'viewer';
  END IF;
  
  -- Add file_url column to files if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'files' AND column_name = 'file_url') THEN
    ALTER TABLE public.files ADD COLUMN file_url text;
  END IF;
END $$;
...
-- Update RLS Policies for files (role-based permissions)
...
CREATE POLICY "Space members can upload files" ON public.files
  FOR INSERT WITH CHECK (
    ...
    EXISTS (
      ...
      AND sm.user_id = auth.uid()
      AND (sm.status = 'active' OR sm.status IS NULL)
      AND (sm.role IN ('admin', 'team_lead', 'employee') OR sm.role IS NULL)
    )
  );
...
DROP POLICY IF EXISTS "Space admins can manage invitations" ON public.space_invitations;
CREATE POLICY "Space admins can manage invitations" ON public.space_invitations
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.space_members sm
    ...
    -- Also allow space creators to manage invitations
    EXISTS (
      SELECT 1 FROM public.spaces s
      WHERE s.id = space_invitations.space_id AND s.created_by = auth.uid()
    )
  );
