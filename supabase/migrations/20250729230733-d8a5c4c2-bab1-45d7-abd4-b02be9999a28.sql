-- Create secure function for team creation with proper authentication
-- This function runs with elevated privileges to handle all team setup operations
...
BEGIN
  ...
  SELECT 
    current_user_id,
    auth.email(),
    COALESCE(user_first_name, auth.raw_user_meta_data()->>'first_name'),
    COALESCE(user_last_name, auth.raw_user_meta_data()->>'last_name')
  WHERE NOT EXISTS (
